...
By Lemma \ref{SimpleGallaiBetaBound},
	...
	which is
	\begin{equation}
	\beta(L) \ge \size{H} + \frac{k}{2}\card{\L} - \size{G}.
	\label{BetaBound}
	\end{equation}
	...
	&\ge (k-2)\card{G} + M + (k-1)\parens{\size{H} + \frac{k}{2}\card{\L} - \size{G}} + 1\\
	&= (k-2)\card{G} + M + (k-1)\size{H} + \frac{k(k-1)}{2}\card{\L} - (k-1)\size{G} + 1.
	\end{align*}		
	Hence
	\begin{equation}
	(k+1)\size{G} \ge (k-2)\card{G} + M + (k-1)\size{H} + \frac{k(k-1)}{2}\card{\L} + 1
	\label{KPOBound}
	\end{equation}
