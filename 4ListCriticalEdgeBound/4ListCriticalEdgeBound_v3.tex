...
\begin{lem}\label{SimpleGallaiBetaBound}
	...
	\[2\size{T} \le (k-2)\card{T} + 2\beta(T).\]
\end{lem}
\begin{proof}
...
\begin{lem}\label{CompicatedGallaiBetaBound}
	 Let $\func{p}{\IN}{\IR}$ with $\frac{2}{k-2} \le p(k) \le 1$ for all $k \in \IN$. If $k \ge 4$ and $T \ne K_k$ is a Gallai tree with maximum degree at most $k-1$, then
	\[2\size{T} \le (k-3 + p(k))\card{T} + (k-1)(1-p(k)) + (2 + (k-1)(1-p(k)))\beta(T).\]
\end{lem}

Note that this bound will pick up an extra $(k-1)(1-p(k))$ per component in a Gallai forest.
...
By Lemma \ref{SimpleGallaiBetaBound},
	\begin{equation*}
	2\size{\L} \le (k-3 + p(k))\card{\L} + (k-1)(1-p(k))c(\L) + (2 + (k-1)(1-p(k)))\beta(\L).
	\end{equation*}
	Hence,
	\begin{align*}
	2\size{G} &= 2\size{\HH} + 2\size{\HH, \L} + 2\size{\L}\\
	&= 2\size{\HH} + 2((k-1)\card{\L} - 2\size{\L}) + 2\size{\L}\\
	&= 2\size{\HH} + 2(k-1)\card{\L} - 2\size{\L}\\
	&\ge 2\size{\HH} + (k+1-p(k))\card{\L} - (k-1)(1-p(k))c(\L) - (2 + (k-1)(1-p(k)))\beta(\L),\\
	\end{align*}
	which is
	\begin{equation}
	\beta(\L) \ge \frac{2\size{\HH} + (k+1-p(k))\card{\L} - (k-1)(1-p(k))c(\L) - 2\size{G}}{2 + (k-1)(1-p(k))}
	\label{CBetaBound}
	\end{equation}
	Let $M$ be the maximum of $\size{I, V(G) \setminus I}$ over all independent sets $I$ of $G$ with $I \subseteq \HH$. Then
	\begin{equation*}
		\mic(G) \ge M + (k-1)\beta(\L).
	\end{equation*}
	To lower bound $M$, consider the components $\C$ of $G[\HH]$.  Now $\alpha(C) \ge \frac{\card{C}}{\chi(C)}$ for all $C \in \C$, so
	\begin{equation}
	  M + (k-1)\size{\HH} \ge \sum_{C \in \C} k\frac{\card{C}}{\chi(C)} + (k-1)\size{C}.
	  \label{CMbound}
	\end{equation}
	If $\L = \emptyset$, then $G$ has average degree at least $k$ which easily satisfies the theorem.  So, assume $\L \ne \emptyset$.  Then $G[\HH]$ is $(k-1)$-colorable by $k$-list-criticality of $G$. 
	In particular, $\chi(C) \le k-1$ for every $C \in \C$.
	For every $C \in \C$,
	\begin{equation}
	 k\frac{\card{C}}{\chi(C)} + (k-1)\size{C} \ge \parens{k - \frac12}\card{C}.
	 \label{CKFC}
	\end{equation}
	To see this, first suppose $C \in \C$ is not a tree. Then $\size{C} \ge \card{C}$ and hence $k\frac{\card{C}}{\chi(C)} + (k-1)\size{C} \ge k\frac{\card{C}}{k-1} + (k-1)\card{C} \ge (k - \frac12)\card{C}$.  
	If $C$ is a tree, then $\chi(C) \le 2$ and hence 
	$k\frac{\card{C}}{\chi(C)} + (k-1)\size{C} \ge k\frac{\card{C}}{2} + (k-1)(\card{C} - 1) \ge (k-\frac12)\card{C}$ unless $\card{C} = 1$.  
	This proves (\ref{CKFC}) since the bound is trivially satisfied when $\card{C} = 1$.  Therefore,
	\begin{equation}
	M + (k-1)\size{H}\ge \parens{k - \frac12}\card{H}.
	\label{CKFH}
	\end{equation}
	Applying Lemma \ref{ConsantListMicStrength} and using (\ref{CBetaBound}) gives
	\begin{align*}
	2\size{G} &\ge (k-2)\card{G} + M + (k-1)\beta(\L) + 1\\
	&\ge (k-2)\card{G} + M + (k-1)\frac{2\size{\HH} + (k+1-p(k))\card{\L} - (k-1)(1-p(k))c(\L) - 2\size{G}}{2 + (k-1)(1-p(k))} + 1\\
	\end{align*}		
	Hence
	\begin{equation}
	X \ge (k-2)\card{G} + M + \frac{2(k-1)}{2 + (k-1)(1-p(k))}\size{\HH} + \frac{(k-1)(k+1-p(k))}{2 + (k-1)(1-p(k))}\card{\L} - J.
	\label{CKPOBound}
	\end{equation}
	where
	\[X = 2\size{G}\parens{1 + \frac{k-1}{2 + (k-1)(1-p(k))}},\]
	and
	\[J = \frac{(k-1)^2(1-p(k))c(\L)}{2 + (k-1)(1-p(k))} - 1.\]
	Now, by (\ref{CKFH}),
	\[M + \frac{2(k-1)}{2 + (k-1)(1-p(k))}\size{\HH} \ge \frac{2\parens{k-\frac12} + k(1-p(k))}{2 + (k-1)(1-p(k))}\card{\HH}\]
	With (\ref{CKPOBound}) this gives
	\begin{equation*}
	X \ge (k-2)\card{G} + \frac{2k-1 + k(1-p(k))}{2 + (k-1)(1-p(k))}\card{\HH} + \frac{(k-1)(k+1-p(k))}{2 + (k-1)(1-p(k))}\card{\L} - J.
	\end{equation*}
	Using $\card{G} = \card{\HH} + \card{\L}$, this becomes
	\begin{equation*}
	X \ge \parens{k-2 + \frac{2k-1 + k(1-p(k))}{2 + (k-1)(1-p(k))}}\card{G} + \frac{k^2 - 2k -(k-p(k))}{2 + (k-1)(1-p(k))}\card{\L} - J.
	\end{equation*}
	Now using the basic bound
	\begin{equation*}
	  \card{\L} \ge k\card{G} - 2\size{G},
	\end{equation*}	
	gives
	\begin{equation*}
	X \ge \parens{k-2 + \frac{2k-1 + k(1-p(k))}{2 + (k-1)(1-p(k))}}\card{G} + \frac{k^2 - 2k -(k-p(k))}{2 + (k-1)(1-p(k))}\parens{k\card{G} - 2\size{G}} - J,
	\end{equation*}
	so
	\begin{equation*}
	2\size{G}\parens{1 + \frac{k^2 - 2k  + p(k) - 1}{2 + (k-1)(1-p(k))}} \ge \parens{k-2 + \frac{3k-kp(k) - 1 + k(k^2 - 2k -(k-p(k)))}{2 + (k-1)(1-p(k))}}\card{G} - J,
	\end{equation*}
	After some algebra, this becomes
	\begin{equation*}
		2\size{G} \ge \parens{k-1 + \frac{k-3}{k^2 - k(p(k) + 1) + 2p(k)}}\card{G} - \frac{J}{ZZ}.
	\end{equation*}
	That proves the theorem.
\end{proof}
