...
multigraph, then $\chi(Q)\le \max\set{\omega(Q),\ceil{\frac{5\Delta(Q)+3}{6}}}$.
\end{thm}
\begin{proof}
Suppose the theorem is false and choose a counterexample $Q$ minimizing $\card{Q}$.  Then $Q = L(G)$ for a critical multigraph $G$.  Say $\chi(Q) = \chi'(G) = k + 1$.  Then
...
Hence we may apply Corollary \ref{CriticalElementary56} to conclude that there are at most two $x \in V(G)$ with $\card{N(x)} \ge 3$
and if there are two such $x_1, x_2$, then $\card{N(x_1)} = \card{N(x_2)} = 3$ and $x_1 \adj x_2$.
