...
In terms of $\gamma_r$, the local version of Reed's conjecture for line graphs proved by Chudnovsky et al. and the superlocal version proved by King and Edwards are the following two theorems.
...
King and Edwards conjectured the fractional version of the following bound without the round-up for all graphs.
\begin{conj}\label{SuperDuperLocalReed}
If $Q$ is a line graph, then $\chi(Q) \le \ceil{\gamma_\infty(Q)}$.
\end{conj}

It is not obvious that $\gamma_r$ is monotone, so let's prove that first.

\begin{lem}\label{MonotoneHelper}
If $A$ and $B$ are graphs with $A \subseteq B$ and $r \in \IN \cup \set{\infty}$, then $\gamma_r(A) \le \gamma_r(B)$.
\end{lem}
\begin{proof}
\textcolor{red}{This is true for all graphs on at most 8 vertices and line graphs of graphs on at most 8 vertices.  Don't have a proof yet.
Is this obvious?}
\end{proof}
...
Say $Q = L(G)$.  Minimality of $\card{Q}$ and Lemma \ref{MonotoneHelper} imply that $G$ is critical.  So Lemma \ref{SuperDuperLocalReedHelper} gives
a contradiction unless $G$ is a thickened cycle.  \textcolor{red}{We should be able to finish, for example if King and Edwards conjecture is true 
