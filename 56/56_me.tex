...
\end{lem}
\begin{proof}
Choose $\alpha \in \vphn(v_0)$ and $\beta \in \vphn(v_1)$ so that $P = P_{v_1}(\alpha, \beta) = v_1v_2\cdots v_rv_0$ contains the maximum number of non-special vertices and subject to that,
maximizes $|P|$.  Then $P$ contains $3$ non-special vertices since otherwise we can use the argument in Lemma \ref{HelperFive}.  By Lemma \ref{HelperFour} and symmetry we may assume that $v_a$, $v_{a+1}$ and $v_b$ are non-special where $a+1 < b$.
By Lemma \ref{FreeColorsLemma}, $V(P) \subsetneq V(T)$.  Hence there are $j,\ell \in \irange{r}$ and $\tau \in \vphn(v_j)$ such that $\vph(v_\ell z) = \tau$ for some $z \in V(T) \setminus V(P)$.
By rotating the $\alpha-\beta$ coloring on $P$, we may assume that $j=0$.  Let $Q = P_{v1}(\tau, \beta}$.  Let $b' = b$ if $b$ is even and $b' = a+1$ if $b$ is odd.

\claim{1}{There is a $\tau$ edge between $v_{t}$ and $v_{t+1}$ for all odd $t$ with $t < a$ or $t \ge b'$.  In particular, $v_a \in V(Q)$.}

\begin{claimproof}
Immediate from Lemma \ref{SpecialPath}.
\end{claimproof}

\claim{2}{$a \le \ell \le b$}

\begin{claimproof}
By Claim 1, each $v_t$ with $0 < t < a$ or $t > b$ is incident to a $\tau$ edge contained in $P$, so $a \le \ell \le b$.
\end{claimproof}

\claim{3}{$b > a+2$, in particular $|P| \ge 5$.}

\begin{claimproof}
Suppose $b = a+2$. By symmetry, we may assume that $a$ is odd.  By Claim 1 every vertex except $v_0$ and possibly $v_{a}$, $v_{a+1}$ is incident to a $\tau$-edge in $G[V(T)]$.  
So $\ell \in \set{a,a+1}$.  The only possibility is that $Q$ contains a $\tau-\beta$ path of length at least three leaving $P$ at $v_a$ and returning at $v_{a+1}$.  Since there is a $\beta$-edge between
$v_{a+1}$ and $v_{a+2}$, this means that $Q$ contains three non-special vertices and $|Q| > |P|$, contradicting our maximality condition on $P$.
\end{claimproof}
