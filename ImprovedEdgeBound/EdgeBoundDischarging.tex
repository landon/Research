...
Instead, let's make $p(k)$ as small as Lemma \ref{BoundFamily} will let us. By (6), $h(k) \le (k-2)p(k) - 2$, plugging this in to (7) and solving we get $p(k) \ge \frac{3k-5}{(k-5)(k-1)}$.  Now $\frac{3k-5}{(k-5)(k-1)} \ge \frac{3}{k-2}$ for $k \ge 6$, so $p(k) = \frac{3k-5}{(k-5)(k-1)}$ satisfies (5).  With $h(k) = \frac{k(k-7)}{(k-5)(k-3)}$, (6) and (7) are also satisfied. Now with $f(k) = \frac{-2(2k^2-15k+15)}{(k-5)(k-3)}$, all the conditions of Lemma \ref{BoundFamily} are satisfied and hence we have the following.

\begin{cor}\label{SmallP}
	For $k \ge 7$ and $T \in \T_k$, we have
	\[2\size{T} \le \parens{k-3 + \frac{3k-5}{(k-5)(k-1)}}\card{T} - \frac{2(2k^2-15k+15)}{(k-5)(k-3)} + \frac{k(k-7)}{(k-5)(k-3)}q(T).\]
\end{cor}
...
Lemma \ref{SmallP}, that is, $p(k) = \frac{3k-5}{(k-5)(k-1)}$, $f(k) = \frac{-2(2k^2-15k+15)}{(k-5)(k-3)}$ and $h(k) = \frac{k(k-7)}{(k-5)(k-3)}$.  The hypotheses of Theorem \ref{UberTheorem} are satisfied with $K=7$ and we get
 \[d(G) \ge k-1 + \frac{2(k-3)(k-4)}{k^3 - 23k + 30}.\]
 
This is better than the bound with Hal for $k \ge 13$.  We can probably take a slightly larger value of $p(k)$ and smaller value of $h(k)$ to do better for the smaller $k$.
