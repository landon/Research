...
%proper subgraph of $G$ is $(k-1)$-choosable. Table \ref{TheTable} shows some history of lower bounds on average degree of $k$-list-critical graphs.
...
%HK We use "k-list-critical" in the table but do not define it.  I think we should have defined it, but if we do we should go through the paper and use it starting with the first sentence after the definition.  Also we should define and use "online k-list-critical 
...
Observe that if a graph is online $f$-choosable then it is $f$-choosable. 
 When $f(v) \DefinedAs k-1$ for all $v \in V(G)$, we say that $G$ is \emph{online $k$-list-critical} if $G$ is not online $f$-choosable, 
  but every proper subgraph $H$ of $G$ is online $\restr{f}{V(H)}$-choosable.  In 2012, Riasat and Schauz \cite{riasat2012critically} showed that Gallai's bound  $2\size{G} \ge g_k(\card{G}, 0)$ holds for online $k$-list-critical graphs.  We improve this for $k \ge 7$ %HK changed 8 to 7
