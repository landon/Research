...
\begin{abstract}
		...
		The same bounds hold for offline $k$-list-critical graphs.
\end{abstract}
...
That means the same proof shows that Main Theorem holds for offline $k$-list-critical graphs as well (in fact, for the larger class of $OC$-irreducible graphs with $\delta(G) = k-1$ defined in section 5).
...
\begin{lem}[Kernel Magic]\label{ConsantListMicStrength} 
...
\noindent\claim{2}{Lemma \ref{betaLemma} is true.}
	...
	\begin{equation}
	 ...
	 \label{eq19}
	\end{equation}
	Applying Lemma \ref{ConsantListMicStrength} using \eqref{eq19} and solving for $\beta(\L)$ proves the claim.
\end{proof}
