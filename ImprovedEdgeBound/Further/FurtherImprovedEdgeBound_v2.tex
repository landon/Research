...
\parens{1 + h(k)}(2\size{G}) \ge \parens{k+1 +(k-1)h(k)- p(k)}\card{\L} + (k- 3)h(k)\card{\HH^-}  - 2\beta(\L)
\end{equation}
...
&+ (k- 3)h(k)\card{\HH^-}  - 2\size{\HH} \\
&+\frac{2(k-2)}{k-1}\card{G} + \frac{2k-1}{k-1}\card{\HH}.
\end{align*}
\end{equation}
...
&- 2\size{\HH} +\parens{(k+1)(k-3)h(k) + \frac{4k-5}{k-1}}\card{G}.
\end{align*}
\end{equation}
\end{proof}
