...
\end{equation}
Since \parens{p,f,h,z}$ is a Gallai bound,
\begin{equation}\label{eq5c}
2\size{\L} \le \parens{k-3 + p(k)}\card{\L} - f(k)c(\L) + h(k)q(\L) + z(k)\beta(\L)
\end{equation}
...
\begin{thm}
	Let $\parens{p,f,h,z}$ be a Gallai bound.  If $k \ge 5$ and $G$ is a non-complete $k$-list-critical graph, then 
	...
	where
	\[S_1 \DefinedAs h(k)\card{\D} - f(k)c(\L),\]
	\[S_2 \DefinedAs (z(k) - 2)\size{H}.\]
\end{thm}
\begin{proof}
