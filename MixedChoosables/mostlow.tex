...
\begin{figure}[htb]
	...
	\Edge[style = _BoldEdgeStyle, label = \small {}, labelstyle={auto=right, fill=none}](v1)(v2)
	\Edge[style = _BoldEdgeStyle, label = \small {}, labelstyle={auto=right, fill=none}](v3)(v2)
	\Edge[style = _BoldEdgeStyle, label = \small {}, labelstyle={auto=right, fill=none}](v1)(v3)
	...
	\end{tikzpicture}
	\caption{The seed blocks.  Each bold edge can be removed without making the figure AT.}
	\label{fig:seeds}
\end{figure}
...
Let $\D$ be the smallest collection of pairs $(G,h)$ containing the pairs in Figure \ref{fig:seeds} that is closed under the operation in Corollary \ref{SubdivideConstructor}.  Explicitly, $\D$ is all graphs that can be created from the graphs in Figure \ref{fig:seeds} by replacing each bold edge with an odd length path.  The rightmost graph in Figure \ref{fig:seeds}  (the Moser Spindle) has no bold edges.
