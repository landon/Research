...
For example, we can now represent the reference graph of the Liar paradox as as the self-loop, since $d(L) = L$ and $L$ occurs as a syntactic constituent of itself.
...
%Whether or not two topological spaces are homeomorphic or not is often a difficult question. Given that danger is a topological property it would suffice to prove that one graph was dangerous and the other not. So danger may give some leverage on such questions 
