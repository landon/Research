...
%Choose an
%arbitrary edge $xy\in E(G)$ and let $H=G-xy$.  We will show that $\chi'(H)\le
%4$.  First, suppose that $\delta(H)\le 2$.  If the 2-vertex has a
%$3^-$-neighbors, then $\chi'(H)\le 4$ by Vizing's Adjacency Lemma.  Otherwise,
%the 2-vertex has a 4-neighbor, call it $v$.  Since $v$ has a 2-neighbor and at
%most two 4-neighbors, again $\chi'(H)\le 4$ by Vizing's Adjacency Lemma.
%First, we show that $H\in \H_4$.
%If
%$\chi'(G-xy) = 5$ for some $xy \in E(G)$, then by minimality of $\size{G}$ some
%component of $G-xy$ is $K_5-e$.  But this is impossible since then one of $x$
%or $y$ has at least three $4$-neighbors.  Hence $G$ is critical. 
