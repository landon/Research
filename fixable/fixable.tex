...
All multigraphs are loopless.  Let $G$ be a multigraph and $L$ a list assignment on $V(G)$.  An \emph{$L$-pot} is a set $X$ containing $\bigcup_{v\in V(G)} L(v)$.
...
When $G$ is $(L, P)$-fixable, the choices of $a,b$ and $J$ in each application of (2) determine a tree where all leaves have lists satisfying (1).  The \emph{height} of $(L, P)$ is the minimum possible height of such a tree.  We write $h_G(L, P)$ for this height and let $h_G(L, P) = \infty$ when $G$ is not $(L,P)$-fixable. 
...
For $i \in \irange{2}$, let $G_i$ be a multigraph, $L_i$ a list assignment on $V(G_i)$ and $P_i$ an $L_i$-pot.
...
\begin{lem}
Suppose $G_i$ is $(L_i, P_i)$-fixable for $i \in \irange{2}$.  If $P_1 \cap P_2 = \emptyset$, then $G_1 \cup G_2$ is $(L', P_1 \cup P_2)$-fixable where $L'(v) = L_1(v) \cup L_2(v)$ for all $v \in V(G_1) \cap V(G_2)$ and $L'(v) = L_i(v)$ for all $v \in V(G_i) \setminus V(G_{3-i})$ and $i \in \irange{2}$.
\end{lem}
...
If $G$ is a star, then $G$ is $(L, P)$-fixable if and only if $(G, L)$ is superabundant and $|L(v)| \ge d_G(v)$ for all $v \in V(G)$.
\end{thm}
\begin{proof}
\end{proof}
...
\begin{thm}
Suppose $G$ is a star with one edge subdivided and root $r$. If $(G, L)$ is superabundant, $|L(v)| \ge d_G(v)$ for all $v \in V(G)$ and $|L(r)| > d_G(r)$, then $G$ is $(L, P)$-fixable.
\end{thm}
\begin{proof}
...
\section{Conjectures}
\begin{conjecture}
Any multigraph $G$ is $(L, P)$-fixable if $(G, L)$ is superabundant and $|L(v)| > d_G(v)$ for all $v \in V(G)$.
\end{conjecture}

\begin{conjecture}
Any tree $G$ is $(L, P)$-fixable if $(G, L)$ is superabundant and $|L(v)| > d_G(v)$ for all $v \in V(G)$.
\end{conjecture}

\begin{conjecture}
Any path $G$ is $(L, P)$-fixable if $(G, L)$ is superabundant and $|L(v)| > d_G(v)$ for all $v \in V(G)$.
\end{conjecture}
