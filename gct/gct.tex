...
in $H$ would contain $x_v$ and hence give rise to an odd cycle in $G$.  So $H$ contains no odd cycle. Since $|H| < |G|$, appplying the theorem to $H$ gives a 2-coloring of $H$, say with red and blue
...
a maximal independent set in $G$.  Since $S$ is maximal, every vertex in $G-S$ has a neighbor in $S$, so $\Delta(G) \ge \Delta(G-S) + 1$.
...
So, $\Delta(G) + 1 \le \chi(G) \le \chi(G-S) + 1$. We conclude $\chi(G-S) > \Delta(G - S)$.  
Since $\card{G-S} < \card{G}$ applying the theorem to $G-S$ shows that $\Delta(G-S) < 3$ or $\omega(G - S) > \Delta(G -S)$.
