...
When $H \subseteq G$, we say that $G$ \emph{contains} $H$ \aaside{contains}.  If $v \in V(G)$, then $G-v$ is the graph we get by removing $v$ and all edges incident to $v$ from $G$.
