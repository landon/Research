...
$y_0 = y$ and $k < \Delta(G)$.  Say $\pi(xy_i) = i + 2$ for $i \in \irange{k}$.  Each of $x$ and $y_0$ is incident to at most $\Delta(G) - 1$ colored edges, so
each has at least two available colors.  By symmetry, we may assume that $1,2$ are available at $x$ and $3,4$ are available at $y_0$.
\end{proof}
