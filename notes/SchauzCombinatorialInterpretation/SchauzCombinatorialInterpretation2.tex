...
\begin{enumerate}
	...
	\item[(c)] if $h(S) = 0$, then $S$ is independent; and
...
\end{lem}
\begin{proof}
	Put $F = E(S, V(G) \setminus S)$. Since $h(S) = 0$, tame part (c) implies that $S$ is independent. Apply tame part (b) repeatedly to the edges in $F$ to get $g \in \IN^{V(G)}$ such that
	\begin{itemize}
