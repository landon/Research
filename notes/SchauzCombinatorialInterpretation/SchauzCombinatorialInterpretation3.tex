...
For a set $S$, let $2^S$ be the power set of $S$. We consider functions $\rho$ that take a graph $G$ and $h \in \IN^{V(G)}$, where $\rho(G,h) \in 2^{V(G)}$.  The end goal is to prove results of the form:
