...
\begin{Lemma}
If $G$ is a connected graph, then for every $X \subseteq V(G)$ at least one of the following holds:
\begin{enumerate}
...
Suppose the Lemma is false and choose a counterexample $G$ and $X \subseteq G$ minimizing $|G|$ and subject to that, minimizing $|X|$.
\end{proof}
