...
In fact, Stiebitz proved a stronger statement.  Theorem follows immediately from Lemma \ref{MainLemma} using $X = V(\L(G))$.  
...
\begin{lem}\label{MagicVertexExists}
Let $G$ be a bipartite graph with parts $A \ne \emptyset$ and $B$ such that $G$ has positive surplus with respect to $A$.   Then there is $x \in A$ such that removing any set of at most $d_G(x) - 2$ edges incident to $x$ leaves a bipartite graph having positive surplus with respect to $A$.
\end{lem}
\begin{proof}
\end{proof}

This same special vertex in $A$ has another property that we need in the induction step for Lemma \ref{WithPositiveSurplus}.
